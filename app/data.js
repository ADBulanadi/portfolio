...
export const getRecentUserActivity = async (username) => {
  const timerLabel = `getRecentUserActivity-${username}-${Date.now()}`;

  console.log(
    `[${new Date().toISOString()}] Fetching recent activity for`,
    username
  );
  console.time(timerLabel); // Start a unique timer
  ...
  try {
    ...
    console.timeEnd(timerLabel); // Always stop the timer
  }
};
